...
class Madek {
    ...
    connect() {
        if (this.config.getToken() === '') {
            throw new Error('Brak Tokena. Nie mogę\'się połączyć bez' +
                ' tokena. Ustaw w pliku konfiguracji.');
        }

        this.client = new Discord.Client();

        this.client.login(this.config.getToken()).then(() => {
            logger.log('info', 'Madek jest gotowy do pracy!');
        }).catch((err) => {
            logger.log('error',
                'Wystąpił błąd połączenia z Discord:' +
                ' %s', err.message);
        });
    }
