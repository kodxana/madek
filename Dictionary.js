...
class Dictionary {
    ...
    load() {
        ...
        try {
            ...
            if (err.code === 'ENOENT') {
                logger.log('warn', ' Nie znaleziono lines.txt. If this is a' +
        ...
        logger.log('info',
            'Załadowano! Znam ' + this.lines.size + ' zdań' +
            ' i ' + this.words.size + ' słów.');
    }
    ...
    save() {
        ...
        fs.writeFile(linesPath, data, (err) => {
            if (err) throw err;
            logger.log('info', 'Zapisano słownik!');
        });
    }
    ...
    learn(str) {
        ...
        if (_.intersection(words, this.config.getBlacklistedWords()).length !== 0) {
            logger.log('debug', '\"%s\" zawiera zakazane słowo.' +
                ' Odmowa nauki.', str);
            return;
        }
